...
public class GameManager : MonoBehaviour
{
    ...
    public void GateChargerLevelComplete(int bonusPointCount)
    {
        if (!(isLevelCompleted || isLevelFailed))
        {
            UIManager.instance.GateChargerLevelCompleteForUI(.5f, bonusPointCount);
            currentLevel++;
            gemCountTotalTemp = gemCountTotal * bonusPointCount;
            gemCountTotal += gemCountInLevel * bonusPointCount;
            Save();
            isLevelCompleted = true;
        }
    }
    ...
    public void CollectGem(int gemCount)
    {
        gemCountInLevel += gemCount;
