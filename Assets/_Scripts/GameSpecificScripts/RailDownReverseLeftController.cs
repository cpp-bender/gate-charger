...
public class RailDownReverseLeftController : MonoBehaviour
{
    ...
    private int takenBallCount;
    private float moveTweenTime = .1f;

    [Space(5)]
    public GameObject railDownReverseInstantiatedLeftBall;
    public GameObject playerReverseInstantiatedLeftBall;
    ...
    private void OnTriggerEnter(Collider other)
    {
        if (other.CompareTag(Tags.StackedBall))
        {
            ...
            cloneBall = Instantiate(railDownReverseInstantiatedLeftBall);

            if (takenBallCount == 0)
            {
                cloneBall.GetComponent<RailDownBall>().shouldStop = true;
        ...
        if (other.CompareTag(Tags.Player))
        {
            if (stackManager.ballCount == 0)
            {
                playerCloneBall = Instantiate(playerReverseInstantiatedLeftBall);
