...
public class RailDownRightController : MonoBehaviour
{
    ...
    [Space(5)]
    public GameObject railDownInstantiatedRightBall;
    public GameObject playerInstantiatedRightBall;
    ...
    private void OnTriggerEnter(Collider other)
    {
        if (other.CompareTag(Tags.StackedBall))
        {
            ...
            cloneBall = Instantiate(railDownInstantiatedRightBall);
        ...
        if (other.CompareTag(Tags.Player))
        {
            if (stackManager.ballCount == 0)
            {
                playerCloneBall = Instantiate(playerInstantiatedRightBall);
